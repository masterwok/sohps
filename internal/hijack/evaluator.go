...
func evaluateHijackVector(targetPath, dirPath string, fileExists bool) (bool, string) {
	if fileExists {
		return evaluateExistingFileVector(targetPath, dirPath)
	}
	return evaluateMissingFileVector(targetPath, dirPath)
}

// evaluateExistingFileVector determines hijack strategies when the target library already exists.
func evaluateExistingFileVector(targetPath, dirPath string) (bool, string) {
	...
	if parentDir, found := fs.FindWritableParent(dirPath); found {
...
func evaluateMissingFileVector(targetPath, dirPath string) (bool, string) {
